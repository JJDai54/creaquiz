 ...
 * @return array
 */
function b_creaquiz_categories_show($options)
{
	include_once XOOPS_ROOT_PATH . '/modules/creaquiz/class/categories.php';
 ...
 * @return string
 */
function b_creaquiz_categories_edit($options)
{
	include_once XOOPS_ROOT_PATH . '/modules/creaquiz/class/categories.php';
	$creaquizHelper = Helper::getInstance();
	$categoriesHandler = $creaquizHelper->getHandler('Categories');
	$GLOBALS['xoopsTpl']->assign('creaquiz_upload_url', CREAQUIZ_UPLOAD_URL);
	$form = _MB_CREAQUIZ_DISPLAY;
	$form .= "<input type='hidden' name='options[0]' value='".$options[0]."' />";
	$form .= "<input type='text' name='options[1]' size='5' maxlength='255' value='" . $options[1] . "' />&nbsp;<br>";
	$form .= _MB_CREAQUIZ_TITLE_LENGTH . " : <input type='text' name='options[2]' size='5' maxlength='255' value='" . $options[2] . "' /><br><br>";
	array_shift($options);
	array_shift($options);
	array_shift($options);
	...
	$categoriesAll = $categoriesHandler->getAll($crCategories);
	unset($crCategories);
	$form .= _MB_CREAQUIZ_CATEGORIES_TO_DISPLAY . "<br><select name='options[]' multiple='multiple' size='5'>";
	$form .= "<option value='0' " . (in_array(0, $options) == false ? '' : "selected='selected'") . '>' . _MB_CREAQUIZ_ALL_CATEGORIES . '</option>';
	foreach(array_keys($categoriesAll) as $i) {
