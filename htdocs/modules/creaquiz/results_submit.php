...
// echo "Enregistrement des résultats";
        ...
        //$templateMain = 'creaquiz_admin_result.tpl';
		//$GLOBALS['xoopsTpl']->assign('navigation', $adminObject->displayNavigation('results.php'));
		//$adminObject->addItemButton(_AM_CREAQUIZ_RESULTS_LIST, 'results.php', 'list');
...
//$bolOK = $resultsHandler->insert($resultsObj);exit;
		if ($resultsHandler->insert($resultsObj)) {
			...
			//redirect_header("results.php?op=list&quiz_id={$quizId}&sender={$sender}", 2, _MA_CREAQUIZ_FORM_OK);
			$newResultId = $resultsObj->getNewInsertedIdResults();
            $url = "solutions.php?quiz_id={$quizId}&sender={$sender}&result_id={$newResultId}";
            redirect_header($url, 2, _MA_CREAQUIZ_FORM_OK);
