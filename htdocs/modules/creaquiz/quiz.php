...
$GLOBALS['xoopsOption']['template_main'] = 'creaquiz_quiz.tpl';
...
switch($op) {
	...
	case 'save':
		...
		if ($quizHandler->insert($quizObj)) {
			...
			$grouppermHandler->deleteByModule($mid, 'creaquiz_view_quiz', $newQuizId);
			if (isset($_POST['groups_view_quiz'])) {
				foreach($_POST['groups_view_quiz'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_view_quiz', $newQuizId, $onegroupId, $mid);
				}
			}
			// Permission to submit_quiz
			$grouppermHandler->deleteByModule($mid, 'creaquiz_submit_quiz', $newQuizId);
			if (isset($_POST['groups_submit_quiz'])) {
				foreach($_POST['groups_submit_quiz'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_submit_quiz', $newQuizId, $onegroupId, $mid);
				}
			}
			// Permission to approve_quiz
			$grouppermHandler->deleteByModule($mid, 'creaquiz_approve_quiz', $newQuizId);
			if (isset($_POST['groups_approve_quiz'])) {
				foreach($_POST['groups_approve_quiz'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_approve_quiz', $newQuizId, $onegroupId, $mid);
				}
			}
			// Handle notification
			...
			$tags['ITEM_URL']  = XOOPS_URL . '/modules/creaquiz/quiz.php?op=show&quiz_id=' . $quizId;
			...
			if ('' !== $uploaderErrors) {
				...
				redirect_header('quiz.php?op=list', 2, _MA_CREAQUIZ_FORM_OK);
			}
		}
		// Get Form Error
	...
	case 'edit':
		...
		// Check params
		if (0 == $quizId) {
			redirect_header('quiz.php?op=list', 3, _MA_CREAQUIZ_INVALID_PARAM);
		}
		// Get Form
		$quizObj = $quizHandler->get($quizId);
		$form = $quizObj->getFormQuiz();
	...
	case 'delete':
		...
		if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
			...
			if ($quizHandler->delete($quizObj)) {
				...
				redirect_header('quiz.php', 3, _MA_CREAQUIZ_FORM_DELETE_OK);
			} else {
				$GLOBALS['xoopsTpl']->assign('error', $quizObj->getHtmlErrors());
			}
		} else {
			xoops_confirm(['ok' => 1, 'quiz_id' => $quizId, 'op' => 'delete'], $_SERVER['REQUEST_URI'], sprintf(_MA_CREAQUIZ_FORM_SURE_DELETE, $quizObj->getVar('quiz_cat_id')));
		}
	break;
}
