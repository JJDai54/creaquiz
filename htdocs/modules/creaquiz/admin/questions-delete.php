...
use XoopsModules\Creaquiz\Constants;
  ...
  if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
  	...
  	if ($questionsHandler->deleteCascade($questionsObj)) {
  		redirect_header('questions.php?' . getParams2list($questQuiz_id, $quest_type_question), 3, _AM_CREAQUIZ_FORM_DELETE_OK);
  	} else {
  		$GLOBALS['xoopsTpl']->assign('error', $questionsObj->getHtmlErrors());
  	}
  } else {
  	xoops_confirm(['ok' => 1, 'quest_id' => $questId, 'op' => 'delete'], $_SERVER['REQUEST_URI'], sprintf(_AM_CREAQUIZ_FORM_SURE_DELETE, $questionsObj->getVar('quest_quiz_id')));
  }
