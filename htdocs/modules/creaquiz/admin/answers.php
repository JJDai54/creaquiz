...
switch($op) {
	...
	default:
		...
		$templateMain = 'creaquiz_admin_answers.tpl';
		$GLOBALS['xoopsTpl']->assign('navigation', $adminObject->displayNavigation('answers.php'));
		/*
        $adminObject->addItemButton(_AM_CREAQUIZ_ADD_ANSWERS, 'answers.php?op=new', 'add');
        ...
        */
        ...
        $inpCategory->setExtra(CREAQUIZ_SELECT_ONCHANGE);
        ...
        $inpQuiz->setExtra(CREAQUIZ_SELECT_ONCHANGE);
  	    $GLOBALS['xoopsTpl']->assign('inpQuiz', $inpQuiz->render());
        
        $inpQuest = new \XoopsFormSelect(_AM_CREAQUIZ_QUESTION, 'quest_id', $questId);
        $inpQuest->addOptionArray($questionsHandler->getListKeyName($quizId));
        $inpQuest->setExtra(CREAQUIZ_SELECT_ONCHANGE);
  	    $GLOBALS['xoopsTpl']->assign('inpQuest', $inpQuest->render());
        
       // ----- /Listes de selection pour filtrage -----
/*
        $btnNewAnswer = $quizUtility->getNewBtn(_AM_CREAQUIZ_ADD_NEW_ANSWER, 'new_answer', CREAQUIZ_ICONS_URL."/16/add.png",  _AM_CREAQUIZ_ADD_NEW_ANSWER);
		$GLOBALS['xoopsTpl']->assign('btnNewAnswer', $btnNewAnswer);
        //---------------------------------------------        
        //update weight 
        $initWeight = $quizUtility->getNewBtn(_AM_CREAQUIZ_COMPUTE_WEIGHT, 'init_weight', CREAQUIZ_ICONS_URL."/16/generer-1.png",  _AM_CREAQUIZ_COMPUTE_WEIGHT);
...
*/               
		...
		if ($answersCount > 0) {
			...
			$GLOBALS['xoopsTpl']->assign('error', _AM_CREAQUIZ_THEREARENT_ANSWERS);
		}
	break;
	...
	case 'new':
		$templateMain = 'creaquiz_admin_answers.tpl';
		$GLOBALS['xoopsTpl']->assign('navigation', $adminObject->displayNavigation('answers.php'));
		$adminObject->addItemButton(_AM_CREAQUIZ_ANSWERS_LIST, 'answers.php', 'list');
		$GLOBALS['xoopsTpl']->assign('buttons', $adminObject->displayButton('left'));
		// Form Create
	...
	case 'save':
		...
		$answersObj->setVar('answer_image', Request::getInt('answer_image', ''));
        
		// Insert Data
		if ($answersHandler->insert($answersObj)) {
			redirect_header('answers.php?op=list' . getParams2list($questId), 2, _AM_CREAQUIZ_FORM_OK);
		}
		// Get Form
	...
	case 'delete':
		...
		if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
			...
			if ($answersHandler->delete($answersObj)) {
				redirect_header('answers.php', 3, _AM_CREAQUIZ_FORM_DELETE_OK);
			} else {
				$GLOBALS['xoopsTpl']->assign('error', $answersObj->getHtmlErrors());
			}
		} else {
			xoops_confirm(['ok' => 1, 'answer_id' => $answerId, 'op' => 'delete'], $_SERVER['REQUEST_URI'], sprintf(_AM_CREAQUIZ_FORM_SURE_DELETE, $answersObj->getVar('answer_quest_id')));
		}
	break;
