...
//use JJD;
        ...
        if($newQuiz){
             ...
             $questionsObj->setVar('quest_question', _AM_CREAQUIZ_QUIZ_PRESENTATION);
             ...
             $answerObj->setVar('answer_proposition', _AM_CREAQUIZ_QUIZ_PRESENTATION);
             ...
             $questionsObj->setVar('quest_actif', 1);
             $questionsObj->setVar('quest_parent_id', 0);
             $questionsObj->setVar('quest_question', _AM_CREAQUIZ_QUIZ_RESULTATS);
		     $questionsHandler->insert($questionsObj);      
			 $questId = $questionsObj->getNewInsertedIdQuestions();
             
             $answerObj = $answersHandler->create();
             $answerObj->setVar('answer_quest_id',$questId);
             $answerObj->setVar('answer_proposition', _AM_CREAQUIZ_QUIZ_RESULTATS_DESC);
             $answerObj->setVar('answer_weight',0);
		     $answersHandler->insert($answerObj);  
                        
        }else if ($oldFolder !== $newFolder){
            //Le quiz a changé de dossier qu'il faut renomer
            rename(CREAQUIZ_UPLOAD_QUIZ_PATH . '/' . $oldFolder, CREAQUIZ_UPLOAD_QUIZ_PATH . '/' . $newFolder);
        }
...
/*
            ...
            //$f = XOOPS_ROOT_PATH . "/modules/creaquiz/language/{$lanquage}/slide/slide_resultats.html";
            $f = CREAQUIZ_PATH . "/language/{$lanquage}/slide/slide_resultats.html";
...
*/            
        ...
        //echo "<hr>quiz : {$quizId}<br>newQuizId : {$newQuizId}<hr>";exit;
			...
			$grouppermHandler->deleteByModule($mid, 'creaquiz_view_quiz', $permId);
			if (isset($_POST['groups_view_quiz'])) {
				foreach($_POST['groups_view_quiz'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_view_quiz', $permId, $onegroupId, $mid);
				}
			}
			// Permission to submit_quiz
			$grouppermHandler->deleteByModule($mid, 'creaquiz_submit_quiz', $permId);
			if (isset($_POST['groups_submit_quiz'])) {
				foreach($_POST['groups_submit_quiz'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_submit_quiz', $permId, $onegroupId, $mid);
				}
			}
			// Permission to approve_quiz
			$grouppermHandler->deleteByModule($mid, 'creaquiz_approve_quiz', $permId);
			if (isset($_POST['groups_approve_quiz'])) {
				foreach($_POST['groups_approve_quiz'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_approve_quiz', $permId, $onegroupId, $mid);
				}
			}
            $url = "quiz.php?op=list&quiz_id={$quizId}&sender=";
			redirect_header("quiz.php?op=list&quiz_id={$quizId}&sender=", 2, _AM_CREAQUIZ_FORM_OK);
		}
		// Get Form
