...
if($quizId > 0 && $sender != 'cat_id'){
  ...
  $catId = $quiz->getVar('quiz_cat_id');
    ...
    $quizId = $quizHandler->getFirstIdOfParent($catId);    
		...
		$adminObject->addItemButton(_AM_CREAQUIZ_COMPUTE_WEIGHT, "quiz.php?op=init_weight&cat_id={$catId}", 'update');
...
// 		$GLOBALS['xoopsTpl']->assign('initWeight', $initWeight);
      ...
      $inpCategory->setExtra('onchange="document.creaquiz_select_filter.submit()"');
     ...
     // ----- /Listes de selection pour filtrage -----        
      ...
      $GLOBALS['xoopsTpl']->assign('allCategories', $catId == 0);
		...
		if ($quizCount > 0) {
			...
			$GLOBALS['xoopsTpl']->assign('error', _AM_CREAQUIZ_THEREARENT_QUIZ);
		}
