...
switch($op) {
	...
	default:
		...
		$templateMain = 'creaquiz_admin_results.tpl';
        ///-------------------------------------------------------
       ...
       // ----- /Listes de selection pour filtrage -----    
        $btn['razResults'] = $quizUtility->getNewBtn(_AM_CREAQUIZ_RAZ_RESULTS, 'delete_all', CREAQUIZ_ICONS_URL."/16/delete.png",  _AM_CREAQUIZ_RAZ_RESULTS);
...
// 		$GLOBALS['xoopsTpl']->assign('creaquiz_upload_url', CREAQUIZ_UPLOAD_URL);
		...
		if ($resultsCount > 0) {
			...
			$GLOBALS['xoopsTpl']->assign('error', _AM_CREAQUIZ_THEREARENT_RESULTS);
		}
	break;
	...
	case 'delete':
		...
		if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
			...
			if ($resultsHandler->delete($resultsObj)) {
				redirect_header("results.php?quiz_id={$quizId}", 3, _AM_CREAQUIZ_FORM_DELETE_OK);
			} else {
...
//             $name = $quiz->getVar('quiz_name');
            $msg = sprintf(_AM_CREAQUIZ_FORM_SURE_DELETE, $resultsObj->getVar('result_id'));
	...
	case 'delete_all':
		...
		if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
			...
			redirect_header("results.php?quiz_id={$quizId}", 3, _AM_CREAQUIZ_DELETE_RESULTS_OK);
...
// 			}
            ...
            $msg = sprintf(_AM_CREAQUIZ_CONFIRM_RAS_RESULTS, $name, $quizId); 
            //sprintf(_AM_CREAQUIZ_FORM_SURE_DELETE, $resultsObj->getVar('result_quiz_id'))
