...
switch($op) {
	...
	default:
		...
		$templateMain = 'creaquiz_admin_categories.tpl';
	$adminObject->displayNavigation('categories.php');
		$adminObject->addItemButton(_AM_CREAQUIZ_ADD_CATEGORIES, 'categories.php?op=new', 'add');
		...
		if ($categoriesCount > 0) {
			...
			$GLOBALS['xoopsTpl']->assign('error', _AM_CREAQUIZ_THEREARENT_CATEGORIES);
		}
	break;
	case 'new':
		$templateMain = 'creaquiz_admin_categories.tpl';
		$GLOBALS['xoopsTpl']->assign('navigation', $adminObject->displayNavigation('categories.php'));
		$adminObject->addItemButton(_AM_CREAQUIZ_CATEGORIES_LIST, 'categories.php', 'list');
		$GLOBALS['xoopsTpl']->assign('buttons', $adminObject->displayButton('left'));
		// Form Create
	...
	case 'save':
		...
		if ($categoriesHandler->insert($categoriesObj)) {
			...
			$grouppermHandler->deleteByModule($mid, 'creaquiz_view_categories', $permId);
			if (isset($_POST['groups_view_categories'])) {
				foreach($_POST['groups_view_categories'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_view_categories', $permId, $onegroupId, $mid);
				}
			}
			// Permission to submit_categories
			$grouppermHandler->deleteByModule($mid, 'creaquiz_submit_categories', $permId);
			if (isset($_POST['groups_submit_categories'])) {
				foreach($_POST['groups_submit_categories'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_submit_categories', $permId, $onegroupId, $mid);
				}
			}
			// Permission to approve_categories
			$grouppermHandler->deleteByModule($mid, 'creaquiz_approve_categories', $permId);
			if (isset($_POST['groups_approve_categories'])) {
				foreach($_POST['groups_approve_categories'] as $onegroupId) {
					$grouppermHandler->addRight('creaquiz_approve_categories', $permId, $onegroupId, $mid);
				}
			}
			redirect_header('categories.php?op=list', 2, _AM_CREAQUIZ_FORM_OK);
		}
		// Get Form
	...
	case 'delete':
		...
		if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
			...
			if ($categoriesHandler->delete($categoriesObj)) {
				redirect_header('categories.php', 3, _AM_CREAQUIZ_FORM_DELETE_OK);
			} else {
				$GLOBALS['xoopsTpl']->assign('error', $categoriesObj->getHtmlErrors());
			}
		} else {
			xoops_confirm(['ok' => 1, 'cat_id' => $catId, 'op' => 'delete'], $_SERVER['REQUEST_URI'], sprintf(_AM_CREAQUIZ_FORM_SURE_DELETE, $categoriesObj->getVar('cat_name')));
		}
	break;
