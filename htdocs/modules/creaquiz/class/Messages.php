...
class Messages extends \XoopsObject
{
	...
	public function getFormMessages($action = false)
	{
		...
		// Title
		$title = $this->isNew() ? sprintf(_AM_CREAQUIZ_MESSAGES_ADD) : sprintf(_AM_CREAQUIZ_MESSAGES_EDIT);
		// Get Theme Form
		...
		// Form Text msgCode
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_MESSAGES_CODE, 'msg_code', 100, 80, $this->getVar('msg_code') ), true);
		
        // Form Text msgMessage
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_MESSAGE, 'msg_message', 100, 255, $this->getVar('msg_message') ), true);
        
        // Form Text msgLanguage
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_LANGUAGE, 'msg_language', 100, 50, $this->getVar('msg_language') ), true);
