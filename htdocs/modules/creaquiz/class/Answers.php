...
class Answers extends \XoopsObject
{
	...
	{global $questionsHandler;
		...
		// Title
		$title = $this->isNew() ? sprintf(_AM_CREAQUIZ_ANSWERS_ADD) : sprintf(_AM_CREAQUIZ_ANSWERS_EDIT);
		// Get Theme Form
		...
		$answerQuestion_idSelect = new \XoopsFormSelect( _AM_CREAQUIZ_QUESTION, 'answer_quest_id', $this->getVar('answer_quest_id'));
		...
		//$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_ANSWERS_PROPOSITION, 'answer_proposition', 50, 255, $this->getVar('answer_proposition') ), true);
        //--------------------------------------------------------------
		...
		$form->addElement(new \XoopsFormEditor( _AM_CREAQUIZ_QUESTIONS_COMMENT1, 'answer_proposition', $editorConfigs) );
        //--------------------------------------------------------------
        
		// Form Text answerCaption
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_CAPTION, 'answer_caption', 80, 255, $this->getVar('answer_caption') ), false);
        
		// Form Text answerGroup
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_ANSWERS_GROUP, 'answer_group', 50, 255, $this->getVar('answer_group') ), false);
		// Form Text answerPoints
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_ANSWERS_POINTS, 'answer_points', 50, 255, $this->getVar('answer_points') ), false);
		// Form Text answerWeight
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_WEIGHT, 'answer_weight', 50, 255, $this->getVar('answer_weight') ), false);
		// Form Text answer_inputs
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_INPUTS, 'answer_inputs', 50, 255, $this->getVar('answer_inputs') ), false);
		// Form Text answer_image
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_IMAGE, 'answer_image', 50, 255, $this->getVar('answer_image') ), false);
