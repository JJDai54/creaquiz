 ...
 */
class slide_ulDaDGroups extends XoopsModules\Creaquiz\Type_question
{
	...
	{
        parent::__construct("ulDaDGroups", 0, 100, "ul");
        $this->maxPropositions = 16;	
        $this->optionsDefaults = ['group1'=>'Gauche','group2'=>'Centre','group3'=>'Droite','groupDefault'=>'0'];
    }
...
/* pas vraiment utile
      ...
      $name = 'imgHeight2';
      $inpHeight1 = new \XoopsFormNumber(_AM_CREAQUIZ_SIZE1,  "{$optionName}[{$name}]", $this->lgPoints, $this->lgPoints, $tValues[$name]);
...
*/      
      ...
      $name = 'groupDefault';  
      $inputGroupDefault = new \XoopsFormRadio(_AM_CREAQUIZ_GROUP_DEFAULT, "{$optionName}[{$name}]", $tValues[$name], ' ');
      $inputGroupDefault->addOption(0, _AM_CREAQUIZ_GROUP_ALL);            
...
echo "<hr><pre>options : " . print_r($options, true) . "</pre><hr>";
        ...
        $i = $this->getFormGroup($trayAllAns, 0, $answers, _AM_CREAQUIZ_SEQUENCE, 0, $this->maxPropositions, '', $options);
...
//                $caption     = '';
            ...
            $libGroup0 = _AM_CREAQUIZ_GROUP_ALL;
            ...
            $inpgroup->addOptionArray(['1'=>$libGroup1, '2'=>$libGroup2, '3'=>$libGroup3]);
...
*/
       ...
       foreach ($answers as $key=>$v){
        ...
        if(isset($v['delete_Proposition'])) $answersHandler->deleteId($v['id']);;  
