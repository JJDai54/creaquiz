 ...
 */
class slide_textboxMultiple extends XoopsModules\Creaquiz\Type_question
{
	...
	{
        parent::__construct("textboxMultiple", 0, 500, "textbox");
        $this->optionsDefaults = ['minReponses' => 0];
    }
...
//    echo "<hr>answers<pre>" . print_r($answers, true) . "</pre><hr>";
        ...
        for($h = 0; $h < $this->maxPropositions; $h++){
            ...
            $inpCaption = new XoopsFormText(_AM_CREAQUIZ_SLIDE_TITLE, $name, $this->lgProposition2, $this->lgProposition2, $caption);
            $trayCaption->addElement($inpCaption);
            
            $name = $this->getName($h, 'inputs');
            $inpInputs = new XoopsFormNumber(_AM_CREAQUIZ_SLIDE_INPUTS, $name, $this->lgPoints, $this->lgPoints, $inputs);
            ...
            for($i = 0; $i < $maxMots; $i++){
                ...
                $inpMot = new XoopsFormText(_AM_CREAQUIZ_SLIDE_MOT, $name, $this->lgMot1, $this->lgMot2, $mot);
	...
	foreach(array_keys($answersAll) as $i) {
        ...
        foreach($tp as $key=>$item){
            ...
            if ($points > 0) {
                ...
                $html[] = sprintf($tpl2, $item['exp'], '&nbsp;===>&nbsp;', $points, _CO_CREAQUIZ_POINTS, $color);
            }elseif ($points < 0) {
                $scoreMin += $points;
