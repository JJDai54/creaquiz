 ...
 */
class slide_textareaListbox extends XoopsModules\Creaquiz\Type_question
{
	...
	{
        parent::__construct("textareaListbox", 0, 610, "textarea");
...
* *********************************************************** */
 	public function getFormOptions($caption, $optionName, $jsonValues = null)
 	{
      ...
      $name = 'orientation';  
      $inputOrientation = new \XoopsFormRadio(_AM_CREAQUIZ_ORIENTATION . _AM_CREAQUIZ_PP, "{$optionName}[{$name}]", $tValues[$name], ' ');
...
//       $inpTitle = new \XoopsFormText(_AM_CREAQUIZ_SLIDE_CAPTION0, "{$optionName}[{$name}]", $this->lgProposition, $this->lgProposition, $tValues[$name]);
      ...
      $inpScoreByGoodWord = new \XoopsFormNumber(_AM_CREAQUIZ_SLIDE_001 . _AM_CREAQUIZ_PP,  "{$optionName}[{$name}]",$this->lgPoints,  $this->lgPoints, $tValues[$name]);
      $inpScoreByGoodWord->setMinMax(1, 20);
      $trayOptions->addElement($inpScoreByGoodWord);     
      
      $name = 'scoreByBadWord';  
      $inpScoreByBadWord = new \XoopsFormNumber(_AM_CREAQUIZ_SLIDE_004 . _AM_CREAQUIZ_PP,  "{$optionName}[{$name}]",$this->lgPoints,  $this->lgPoints, $tValues[$name]);
      $inpScoreByBadWord->setMinMax(-10, 0);
      $trayOptions->addElement($inpScoreByBadWord);     
      
      $name = 'minReponses';  
      $inpMinReponses = new XoopsFormNumber(_AM_CREAQUIZ_QUESTIONS_MINREPONSE . _AM_CREAQUIZ_PP,  "{$optionName}[{$name}]", $this->lgPoints, $this->lgPoints, $tValues[$name]);
...
* ************************************************** */
 	public function getForm($questId, $quizId)
 	{
        ...
        $inpPropo = $this->getformTextarea(_AM_CREAQUIZ_QUESTIONS_COMMENT2, $this->getName('proposition'), $proposition,'',8);
        $this->trayGlobal->addElement($inpPropo);   
/*
         $inpPoints = new XoopsFormNumber(_AM_CREAQUIZ_SLIDE_001, $this->getName('points'), $this->lgPoints, $this->lgPoints, $points);
...
*/          
        ...
        $trayAns->addElement(new xoopsFormLabel('', _AM_CREAQUIZ_STRANGER_EXP . ''));
