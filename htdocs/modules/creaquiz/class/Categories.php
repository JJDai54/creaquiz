...
class Categories extends \XoopsObject
{
	...
	public function getFormCategories($action = false)
	{
		...
		// Title
		$title = $this->isNew() ? sprintf(_AM_CREAQUIZ_CATEGORIES_ADD) : sprintf(_AM_CREAQUIZ_CATEGORIES_EDIT);
		// Get Theme Form
		xoops_load('XoopsFormLoader');
		$form = new \XoopsThemeForm($title, 'form', $action, 'post', true);
		$form->setExtra('enctype="multipart/form-data"');
		// Form Text catName
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_CATEGORIES_NAME, 'cat_name', 50, 255, $this->getVar('cat_name') ), true);
		// Form Editor DhtmlTextArea catDescription
		$editorConfigs = [];
		if ($isAdmin) {
			$editor = $creaquizHelper->getConfig('editor_admin');
		} else {
			$editor = $creaquizHelper->getConfig('editor_user');
		}
		...
		$form->addElement(new \XoopsFormEditor( _AM_CREAQUIZ_CATEGORIES_DESCRIPTION, 'cat_description', $editorConfigs) );
        ...
        */
		...
		$inpTheme = new \XoopsFormSelect( _AM_CREAQUIZ_THEME, 'cat_theme', $this->getVar('cat_theme'));
        $inpTheme->setdescription(_AM_CREAQUIZ_THEME_DEFAULT_CAT);
		...
		$form->addElement(new \XoopsFormText( _AM_CREAQUIZ_WEIGHT, 'cat_weight', 20, 50,  $this->getVar('cat_weight')) );
		
        // Form Text Select questTimer
        $inpTimer = new \XoopsFormText( _AM_CREAQUIZ_TIMER, 'quest_timer', 20, 50,  $this->getVar('quest_timer'));
...
/*
		...
		$form->addElement(new \XoopsFormDateTime( _AM_CREAQUIZ_CREATION, 'cat_creation', '', $catCreation ) );
		
        // Form Text Date Select catUpdate
		$catUpdate = $this->isNew() ? 0 : $this->getVar('cat_update');
		$form->addElement(new \XoopsFormDateTime( _AM_CREAQUIZ_UPDATE, 'cat_update', '', $catUpdate ) );
*/		
		...
		if (!$this->isNew()) {
			$groupsIdsApprove = $grouppermHandler->getGroupIds('creaquiz_approve_categories', $this->getVar('cat_id'), $GLOBALS['xoopsModule']->getVar('mid'));
			$groupsIdsApprove[] = array_values($groupsIdsApprove);
			$groupsCanApproveCheckbox = new \XoopsFormCheckBox( _AM_CREAQUIZ_PERMISSIONS_APPROVE, 'groups_approve_categories[]', $groupsIdsApprove);
			$groupsIdsSubmit = $grouppermHandler->getGroupIds('creaquiz_submit_categories', $this->getVar('cat_id'), $GLOBALS['xoopsModule']->getVar('mid'));
			$groupsIdsSubmit[] = array_values($groupsIdsSubmit);
			$groupsCanSubmitCheckbox = new \XoopsFormCheckBox( _AM_CREAQUIZ_PERMISSIONS_SUBMIT, 'groups_submit_categories[]', $groupsIdsSubmit);
			$groupsIdsView = $grouppermHandler->getGroupIds('creaquiz_view_categories', $this->getVar('cat_id'), $GLOBALS['xoopsModule']->getVar('mid'));
			$groupsIdsView[] = array_values($groupsIdsView);
			$groupsCanViewCheckbox = new \XoopsFormCheckBox( _AM_CREAQUIZ_PERMISSIONS_VIEW, 'groups_view_categories[]', $groupsIdsView);
		} else {
			$groupsCanApproveCheckbox = new \XoopsFormCheckBox( _AM_CREAQUIZ_PERMISSIONS_APPROVE, 'groups_approve_categories[]', $fullList);
			$groupsCanSubmitCheckbox = new \XoopsFormCheckBox( _AM_CREAQUIZ_PERMISSIONS_SUBMIT, 'groups_submit_categories[]', $fullList);
			$groupsCanViewCheckbox = new \XoopsFormCheckBox( _AM_CREAQUIZ_PERMISSIONS_VIEW, 'groups_view_categories[]', $fullList);
		}
		// To Approve
