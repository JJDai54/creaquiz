...
class QuizHandler extends \XoopsPersistableObjectHandler
{
	...
	{
		parent::__construct($db, 'creaquiz_quiz', Quiz::class, 'quiz_id', 'quiz_cat_id');
	}
...
//         if ($addNull) $inpList->addOption('_NULL_', _AM_CARTOUCHES_NULL);
        ...
        foreach (array_keys($allAllowed) as $i) {
            ...
            $ret[$key] = $allAllowed[$i]->getVar('quiz_name') . ((CREAQUIZ_ADD_ID) ? " (#{$key})" : "");;
 ...
 * *********************** */
    ...
    global $xoopsDB;
	   ...
	   $sql = "SELECT GROUP_CONCAT(`quest_id`) as questIdList FROM " . $xoopsDB->prefix('creaquiz_questions')
 ...
 * *********************** */
    ...
    {
        global $xoopsUser, $creaquizHelper;
//exit("===>getPermissions : {$short_permtype}");        
        
        $permtype = sprintf("creaquiz_%s_quiz", $short_permtype);
        
        $tPerm = array();
        $creaquizHelper = Helper::getHelper('creaquiz');
        $moduleHandler = $creaquizHelper->getModule();
    ...
    {
        global $categoriesHandler, $creaquizHelper;
        $tPerm = $this->getPermissions($short_permtype);
//exit("===>getAllowed");        
        $idsQuiz = join(',', $tPerm);
        //---------------------------------------------------------------
        if(!$categoriesHandler) $categoriesHandler = $creaquizHelper->getHandler('Categories');
...
// echo "<hr><pre>quiz : " . print_r($quiz, true) . "</pre><hr>";    
    ...
    $imgPath = CREAQUIZ_UPLOAD_QUIZ_PATH . '/' . $folder . '/images';
    //$imgList = XoopsLists::getDirListAsArray(CREAQUIZ_UPLOAD_QUIZ_PATH . '/' . $folder . '/images');
