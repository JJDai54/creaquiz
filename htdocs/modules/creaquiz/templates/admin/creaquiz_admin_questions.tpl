...
function addNewChild(parentId){
  //onclick="document.creaquiz_select_filter.op.value='new_question';document.creaquiz_select_filter.parent_id.value=<{$Questions.id}>;document.creaquiz_select_filter.submit();this.consoleLog(document.creaquiz_select_filter.op.value);">
  document.creaquiz_select_filter.op.value='new';
  document.creaquiz_select_filter.quest_parent_id.value=parentId;
  document.creaquiz_select_filter.submit();
  
  //document.creaquiz_select_filter.op.value + " - " + document.creaquiz_select_filter.quest_parent_id.value)
...
<div class="floatleft">
    <div>
      ...
      <table>
        <tr>
          <td class='right'><{$smarty.const._AM_CREAQUIZ_CATEGORIES}> : </td>
          <td><{$inpCategory}></td>
          <td class='left'></td>
          <td class='right'></td>
        </tr>
        <tr>
          <td class='right'><{$smarty.const._AM_CREAQUIZ_QUIZ}> : </td>
          <td><{$inpQuiz}></td>
          <td class='right'><{$smarty.const._CO_CREAQUIZ_TYPE_QUESTION}> : </td>
          <td><{$inpTypeQuest}></td>
...
<form name='creaquiz_list' id='creaquiz_list' action='questions.php' method='post' enctype=''>
...
<input type="hidden" name="quest_id" value="<{$quest_id}>" />
	...
	<table id='quiz_question_list' name='quiz_question_list' class='table table-bordered'>
		<thead>
			<tr class='head'>
				<th class="center">*</th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_QUESTIONS_ID}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_PARENT_ID}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_QUESTIONS_QUIZ_ID}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_QUESTIONS_TYPE_QUESTION}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_FORM_TYPE_SHORT}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_QUESTIONS_QUESTION}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_WEIGHT}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_QUESTIONS_POINTS}></th>
				<th class="center"><{$smarty.const._AM_CREAQUIZ_CHRONO}></th>
				<{* <th class="center"><{$smarty.const._AM_CREAQUIZ_QUESTIONS_CREATION}></th> *}>
				<th class="center width5"><{$smarty.const._AM_CREAQUIZ_OPTIONS}></th>
				<th class="center width5"><{$smarty.const._AM_CREAQUIZ_ACTIONS}></th>
			</tr>
		</thead>
		<{if $questions_count}>
		<tbody>
			<{foreach item=Questions from=$questions_list name=quest  key=index}>
                <{if $Questions.typeForm == $smarty.const.CREAQUIZ_TYPE_FORM_BEGIN}>
                  <{assign var="fldImg" value="red"}>
                  <{assign var="styleParent" value="style='background:navajowhite;'"}>
                  
                <{elseif $Questions.typeForm == $smarty.const.CREAQUIZ_TYPE_FORM_GROUP}>
                  <{assign var="fldImg" value="red"}>
                  <{assign var="styleParent" value="style='background:lightblue;'"}>
                  
                <{elseif $Questions.typeForm == $smarty.const.CREAQUIZ_TYPE_FORM_END}>
                  <{assign var="fldImg" value="red"}>
  			...
  			<tr class='<{cycle values='odd, even'}>' style='height:40px;'>
                ...
                <{if  $Questions.typeForm == $smarty.const.CREAQUIZ_TYPE_FORM_GROUP OR $Questions.typeForm == $smarty.const.CREAQUIZ_TYPE_FORM_QUESTION}>
...
<{* 
                    <a href="questions.php?op=change_etat&quiz_id=<{$Questions.quiz_id}>&quest_id=<{$Questions.id}>&field=quest_timer&modulo=-10&action=minus"  title='<{$smarty.const._AM_CREAQUIZ_PLUS}>' >
                      <img src="<{$modPathIcon16}>/arrows/<{$fldImg}>/minus.png" title="<{$smarty.const._AM_CREAQUIZ_LAST}>">
                      </a>
                    <{$Questions.quest_timer}>
                    <a href="questions.php?op=change_etat&quiz_id=<{$Questions.quiz_id}>&quest_id=<{$Questions.id}>&field=quest_timer&modulo=10&action=plus"  title='<{$smarty.const._AM_CREAQUIZ_PLUS}>' >
                      <img src="<{$modPathIcon16}>/arrows/<{$fldImg}>/plus.png" title="<{$smarty.const._AM_CREAQUIZ_LAST}>">
                      </a>
*}>                 
                      <{$Questions.inpTimer}>
                </td>
				...
				<td class="center  width10" <{$styleParent}> >
                    <a href="questions.php?op=change_etat&quiz_id=<{$Questions.quiz_id}>&quest_id=<{$Questions.id}>&field=quest_actif" title='<{$smarty.const._AM_CREAQUIZ_ACTIF}>' >
        				<{*  <img src="<{xoModuleIcons16}><{$Questions.actif}>.png" alt="actif" title='<{$smarty.const._AM_CREAQUIZ_ACTIF}>' /> *}>
                        <{$Questions.flags.actif}>
                        </a>|
                    <a href="questions.php?op=change_etat&quiz_id=<{$Questions.quiz_id}>&quest_id=<{$Questions.id}>&field=quest_visible" title='<{$smarty.const._AM_CREAQUIZ_VISIBLE}>' >
        				<{*  <img src="<{xoModuleIcons16}><{$Questions.visible}>.png" alt="Visible" title='<{$smarty.const._AM_CREAQUIZ_VISIBLE}>' /> *}>
                        <{$Questions.flags.visible}>
                        </a>
                        
                    <{if $Questions.isQuestion}>
                        |<a href="questions.php?op=change_etat&quiz_id=<{$Questions.quiz_id}>&quest_id=<{$Questions.id}>&field=quest_numbering&modulo=3"  title='<{$smarty.const._AM_CREAQUIZ_NUMBERING}>' >
                            <{$Questions.flags.numbering}>
                            </a>
<{* 
                        <a href="questions.php?op=change_etat&quiz_id=<{$Questions.quiz_id}>&quest_id=<{$Questions.id}>&field=quest_shuffleAnswers"  title='<{$smarty.const._AM_CREAQUIZ_SHUFFLE_ANS}>' >
                    ...
                    <{else}>
        				<img src="<{$modPathIcon16}>/grey.gif" alt="Visible" title='<{$smarty.const._AM_CREAQUIZ_NOT_QUESTION}>' />
*}>                             
                    <{/if}>
				</td>
