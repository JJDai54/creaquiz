 ...
 * @return string
 */
function creaquiz_block_addCatSelect($cats)
{
	$cat_sql = '(';
	if (is_array($cats)) {
 ...
 */
function creaquizGetMyItemIds($permtype, $dirname)
{
	global $xoopsUser;
	static $permissions = [];
	...
	$creaquizModule = $moduleHandler->getByDirname($dirname);
	$groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
	$grouppermHandler = xoops_getHandler('groupperm');
	$itemIds = $grouppermHandler->getItemIds($permtype, $groups, $creaquizModule->getVar('mid'));
	return $itemIds;
}
 ...
 * @return int
 */
function creaquizNumbersOfEntries($mytree, $results, $entries, $cid)
{
    $count = 0;
...
function creaquizMetaKeywords($content)
{
...
function creaquizMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function creaquiz_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    $creaquizHelper = \XoopsModules\Creaquiz\Helper::getInstance();
    $resultsHandler = $creaquizHelper->getHandler('results');
    $lenght_id = $creaquizHelper->getConfig('lenght_id');
    $rewrite_url = $creaquizHelper->getConfig('rewrite_url');
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = creaquiz_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function creaquiz_Filter($url, $type = '') {

    // Get regular expression from module setting. default setting is : `[^a-z0-9]`i
    $creaquizHelper = \XoopsModules\Creaquiz\Helper::getInstance();
    $resultsHandler = $creaquizHelper->getHandler('results');
    $regular_expression = $creaquizHelper->getConfig('regular_expression');
